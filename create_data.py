...
def action(board, player1, player2):
	...
	if player1.state:
		...
		with open('data/data_minimax_v2.txt', mode='a') as f:
			f.write(str(board) + ",\n")
		row_label, col_label, _ = best_action(board, player1.chess)
		position = row_label * BOARD  + col_label 
		with open('data/labels_minimax_v2.txt', mode='a') as f:
...
def best_action(board , player):
    ...
    for row, col in emp:
        ...
        score = evaluate(board, player, -player) 
        board[row][col] = 0
